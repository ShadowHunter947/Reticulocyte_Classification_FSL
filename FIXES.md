...
| **F** | **Localization (IoU / pointing-game) was computed for FULL only** — you could not see which ablation knob actually improves patch *placement*. | — | **Per-class pointing-game + IoU@0.1/0.25 now emitted for every ablation arm** and written to the per-class CSV; the master ablation table gains `PG%` and `IoU25%` columns, the per-class benchmark gains `PG / IoU10 / IoU25 / locN`. This is the direct patch-quality test AUROC could not provide, now swept across all arms. | Cell 8 §8.2/§8.4/§8.6 |
...
2. Run Cell 8 ablations with `ABL_SEEDS=[42,1,7]`, `ABL_SUBSET_N=400`. Check that
   **FULL now beats `CTRL:random`** (the question the RandomState bug hid), read the
   new **PG% / IoU25%** columns to see which arms actually improve patch placement,
   and read arms **H** (5-way softmax) and **I** (ensemble off) to confirm the v14
   scoring fixes move the metric.
3. All numbers stay diagnostic until Dr. Nimi signs off on the clinical framing.

---

## 5. Ablation table columns (Cell 8)

**Master table** (per arm, seed-averaged): `AUROCmax, AUPRC, PG%, IoU25%, true%,
fb%, abst%, prior%, leak%, band%, sim`.
**Per-class CSV** (`patch_ablations_v14_perclass.csv`), now per arm × seed × class:
`auroc, auroc_topk, auprc, ci_lo, ci_hi, support_pos/neg, causal, pct_qimg_causal,
abstain_pct, true_pct, fallback_pct, prior_pct, leak_pct, mean_sim, spur_in,
loc_n, pointing_game, iou@0.1, iou@0.25`.
Localization columns are populated only for the three classes with NIH GT boxes
(Atelectasis, Cardiomegaly, Effusion→Pleural Effusion); Consolidation and Edema
are `NaN` there by design (no ground-truth boxes exist).

---

## 6. Full code summary — what each of the 8 cells does

The notebook is `domain_adapt_v14_8cell.ipynb`. Default top-to-bottom run executes
the internal smoke test + Stage 2/3; ablations and heavy stages are flag-gated.

- **Cell 1 — Setup & config.** Imports; device/seed; **best-hyperparameter config**
  (baked defaults); scoring knobs, thresholds, patch geometry, persistence params;
  **positive + negative zero-shot prompt pairs** (FIX A); NIH/MIMIC dataset paths
  incl. `BBOX_CSV`; counters/helpers. This is the one cell you edit for a run.
- **Cell 2 — Data structures, anatomy, report parsing.** `QueryPlan / PatchDocument
  / ImagePatchResult` dataclasses; PA/AP anatomy priors + GPU Gaussian heatmaps;
  report section splitting, negation/uncertainty scoping, view detection.
- **Cell 3 — Vocab mining + Stage 2.** Mines per-disease query phrases from training
  reports; label-column resolution; builds one `QueryPlan` per image (rule-based
  findings → anatomical prior + query snippet).
- **Cell 4 — CheXzero + scoring.** Loads the CLIP ViT-B/32 CheXzero checkpoint;
  builds the 5-way class-text matrix (for the GradCAM objective) **and the binary
  pos/neg prompt matrices** (FIX A); fills query vectors; `ViTGradCAM`; the
  **decoupled binary class-prob** scorer and the **whole-image global prob** (FIX B).
- **Cell 5 — Patch utilities.** Square/in-bounds box geometry; patch encoding;
  combined score; GradCAM-in-box; causal containment; `classify_patches_gpu`
  (**binary prob + global ensemble**); spurious-in-anatomy selection; persistent-
  homology causal mask; outside-anatomy (domain-artifact) scan.
- **Cell 6 — Discovery + A\* metrics + visuals.** `discover_patches_for_plan`
  (per-class scales, ensemble threaded through); resumable `run_stage3`; embedding
  split into causal / spur-in / spur-out; **A\* localization** (pointing-game,
  IoU@0.1/0.25 vs `BBox_List_2017`) and **image-level AUROC/AUPRC-vs-prevalence**;
  **GradCAM causal grid** (5 classes) and **bbox-by-bucket** figures.
- **Cell 7 — Run flags + smoke test + driver.** `run_internal_smoke_test()`
  (GPU/data-free validation of every fix); the real Stage 2+3 driver; saves
  causal/spur-in/spur-out banks; then prints the A\* tables and renders the figures.
- **Cell 8 — Ablations + controls.** The `_control_scores` **RandomState→default_rng
  fix**; per-config evaluator now emitting **per-class pointing-game + IoU** as well
  as AUROC/AUPRC; paired dAUROC-vs-FULL bootstrap; grid arms A–I (incl. `H:5way`,
  `I:ensemble`) + negative controls; master/per-class tables and the overall bar chart.

### Data flow
```
CSV + reports ─► Stage 2 (query plans) ─► CheXzero + GradCAM + persistence
   ─► Stage 3 (causal / spurious-in / spurious-out patches)
        ├─► embeddings banks (.pt)   ├─► A* localization + image metrics
        ├─► GradCAM causal grid       └─► bbox-by-bucket figure
   ─► Ablations (arms A–I + controls) ─► AUROC/AUPRC + PG/IoU per class + CSVs
```
